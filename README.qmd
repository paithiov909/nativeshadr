...
* Write a pixel shader as Rcpp codes.
  * A pixel shader here is expected to have the signature `shader(int2 wh, Rcpp::IntegerMatrix nr, Rcpp::List uniforms)`, where `wh` is the position of a pixel in the texture, `nr` is a 'nativeRaster' object that represents the texture, and `uniforms` is a list of uniforms.
