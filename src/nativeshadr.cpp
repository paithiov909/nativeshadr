...
Rcpp::IntegerVector test_as_is(Rcpp::IntegerMatrix nr, Rcpp::List uniforms) {
  auto as_is = [](std::vector<int> xy, Rcpp::IntegerMatrix nr,
                  Rcpp::List uniforms) {
...
uint32_t texture(std::vector<int> xy, Rcpp::IntegerMatrix nr,
                 Rcpp::List uniforms) {
  float2 uv = float2(xy[0], xy[1]) / float2(nr.ncol(), nr.nrow());
