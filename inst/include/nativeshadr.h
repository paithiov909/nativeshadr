...
inline int4 texture_eval(const Rcpp::IntegerMatrix& nr,
  ...
  if (wh.size() != 2) {
    throw std::runtime_error("wh must have length 2");
  }
...
vectorize_shader(
    std::function<uint32_t(std::vector<int>, Rcpp::IntegerMatrix, Rcpp::List)>
        shader) {
  return [shader](Rcpp::IntegerMatrix nr, Rcpp::List uniforms) {
    ...
    for (int i = 0; i < std::get<0>(dim); i++) {    // height
      for (int j = 0; j < std::get<1>(dim); j++) {  // width
        ret[j + i * std::get<1>(dim)] =
            shader(std::vector<int>{j, i}, nr, uniforms);
      }
    }
